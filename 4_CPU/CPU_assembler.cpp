		...
		for (int indx = 0; indx < file_info.num_words; indx++) {
			if     (!strncmp(file_info.text[indx].line, "push",  PUSH_SIZE)) {
				...
				if (!strncmp(file_info.text[indx].line, "jmp",   JMP_SIZE)) {
					...
					printf("file_info.text[indx].line = %s\n", file_info.text[indx].line);

					if (pass == FIRST_PASS) {
						table_labels.check_label(file_info.text[indx].line, POISON_POSITION, FROM_JMP_CMD);
					}
					else
						POINTER_ON_(buffer_data, tmp_IP, int) = table_labels.find_label(file_info.text[indx].line);	
					printf("POINTER_ON_(buffer_data, tmp_IP, int) = %d\n", POINTER_ON_(buffer_data, tmp_IP, int));	
