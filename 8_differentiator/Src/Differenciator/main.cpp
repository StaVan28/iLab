...
/* !TODO
    1) вынести очевидный нью (в списки инициализации)
    2) замнить fseek () -> fstat ()
    3) норм обработка ошибок
    4) заменить num_of_tokens на max_of _tokens
    5) заменить token на node
    6) пересмотр структуры проекты (пернос глобальных структур в diff)
    7) num_symbols_of_file -- наглядная смена класса(!)
    ...
    10) удалить ручками конструкторы копирования операторы присваивания и тд
    11) добавить длинные перемнные
    12) реализовать конструктор копирования
*/